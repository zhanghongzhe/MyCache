...
namespace MyRedis
{
    ...
    /// 2、打开客户端：redis-cli -h host -p port -a password或者直接执行redis-cli
    /// </summary>
    public class Class1
    {
        public void Test()
        {
            ...
            #region string
            client.Add<string>("StringValueTime", "我已设置过期时间噢3秒后会消失", DateTime.Now.AddMilliseconds(3000));
            while (true)
            {
            ...
            {
                Console.WriteLine("Set无序集合ValueA:{0}", item); //出来的结果是无序的
            }
